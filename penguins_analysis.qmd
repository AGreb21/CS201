...
```{r}
#create_report(penguins_data_from_csv)
```
...
```{r}
ggplot(data = penguins_data_from_csv, aes(culmen_depth_mm, fill = species))  +
  geom_histogram(alpha =0.5) +
  theme_bw() +
  scale_fill_viridis_d() +
  labs(title = "Palmer Penguins, Distribution of Culmen Depth",
       subtitle = "3 penguin species observed on three islands in the Palmer Archipelago",
       x = "Culmen Depth (mm)",
       y = "number of penguins")
...
```{r}
ggplot(data = penguins_data_from_csv, aes(culmen_depth_mm, y = body_mass_g, color = species)) +
...
```{r}
ggplot(average_species_flipper_length, aes(x = species, y = average_flipper_length, fill = species)) +
  geom_bar(stat = "identity") +
  theme_bw() +
  scale_fill_viridis_d() +
