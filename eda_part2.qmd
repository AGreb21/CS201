...
-   Whiskers: min and max excluding outliers +/- 1.5 standard deviations
...
-   \|\> is also called Base R pipe or native pipe
...
ggplot(data = diamonds, mapping = aes(x = carat, y = price)) +
  geom_boxplot(mapping = aes(group = cut_width(carat, 0.25)), orientation = "x")
...
very weak relation of As color improves price decreases (because carat isn't factored)
...
What is the relationship here? As clarity increases, price decreases (because carat is not factored in)

For color and clarity, is there more variation or covariation?

Between color, clarity, and carat, what is the best predictor of diamond prices? carat
...
-   Where do we see a lot of variability? Fair
