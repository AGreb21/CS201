...
-   Does your visualization match your expectations? Yes this visualization looks as I would expect

-   Which values are the most common? Why? values at intervals of 5 are way more common, thats just how they made the pokemon stats
...
ggplot(pokedata, aes(x = sp_atk)) + 
  geom_histogram() +
  coord_cartesian(ylim = c(0, 20), xlim = c(150, 200))
```
...
the line is the median, box encloses the inner quartile range which means the point at 1 quarter and 3 quarters of the data and the "whiskers" extend to the minimum and maximum values excluding outliers that are shown as points.
